...
import { getTemplatePermissionByToken, updateTemplatePermissionByToken } from 'services/dbhandler';
...
const Invitation: React.FC<InvitationProps> = ({}) => {
	...
	const handleClickAcceptButton = async () => {
		...
		const newPermission = await updateTemplatePermissionByToken(
			router.query.invitationToken as string,
			{ status: 'accepted' },
			null,
	...
	const fetchData = async () => {
		setIsLoading(true);
		const permission = await getTemplatePermissionByToken(router.query.invitationToken as string);
		...
		if (permission.status === 'accepted' && currentUser.id == permission.user) {
			InvitationContent = (
				...
				>
					...
					<Box>
						<Button onClick={async () => await router.replace(`/templates/${permission.template}`)}>
							Visit Template
						</Button>
					</Box>
				</Flex>
		...
		if (currentUser.id != permission.user) {
			InvitationContent = (
				...
				>
					...
					<Box>
						<Paragraph>
							If you want to accept this invitation. Please <Link href='/'>log out</Link>
						</Paragraph>
					</Box>
				</Flex>
			);
